...
Path ( or 'quit' to exit)";
	...
	if ( ( $GLOBAL::directory =~ /\/$/ ) or ( $GLOBAL::directory =~ /\\$/ ) ) {
		...
		if ( opendir(DIRECTORY, "$GLOBAL::directory") ) {
			...
			mkdir "$GLOBAL::directory"."800jpg" or warn "\n\tThe directory '$GLOBAL::directory"."800jpg' already exists. Writing to the existing directory.";
		}
		else {
		...
		if (open FILEHANDLE, "$GLOBAL::directory") {
			...
			$singlefile = 'true';
#			mkdir "$GLOBAL::directory/800jpgs" or warn "\n\tThe directory '$GLOBAL::directory/800jpgs' already exists. Writing to the existing directory.";
		}
		else {
	...
	foreach my $filename (@files) {
		...
		if ( ($filename =~ /\.tif/) or ($filename =~ /\.jpg/) or ($filename =~ /\.JPG/) or ($filename =~ /\.gif/) ) {
...
#				warn "$x" if "$x";
			...
			if ( $singlefile ) { 
				...
				$path = $GLOBAL::directory .'800jpg/'. $filename;
			}
			$path =~ s/\.\w\w\w$/\.jpg/;
			print "\tSaving $path"."\n";
			$x = $image->Write("$path");
