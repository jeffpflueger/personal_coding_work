...
my $pages_only = undef; # remake the html pages from the templates, and keep the database intact
my $images_only = undef; # remake the images, but keep the database intact

GetOptions ( pages_only => \$pages_only, images_only => \$images_only  );

#print "pages_only  " . $pages_only . "\n";
...
open_db("dameasy_new", "jeff", "45grimrun" ) || die "Error opening database: $DBI::errstr\n";
...
my $dameasy_image_dir = "photo/picture/image/";
## directory within the dameasy directory for the pages
my $dameasy_page_dir = "photography/pictures/images/";
...
my $ftp_path = "/home/dameasy/ftp_images/";
...
## absolute path to the directory within your webdirectory where the processed images and pages will reside temporarilly
my $tmp_dameasy_path = $absolute . $tmp_html . $dameasy_dir;
...
my $tmp_html_path = $tmp_dameasy_path . $dameasy_page_dir;
...
my $image_path = $dameasy_path . $dameasy_image_dir;
	...
	my $keyword_doc_path = "/" . $dameasy_dir . "photography/pictures/images_keywords/";
	
	my $html_keywords_path = $dameasy_path . "photography/pictures/images_keywords/";
	my $tmp_html_keywords_path = $tmp_dameasy_path . "photography/pictures/images_keywords/";
...
my $flash_xml_all_template_path = $dameasy_path . $dameasy_template_dir . 'images_all.php';
my $swf_all_template_path = $dameasy_path . $dameasy_template_dir . 'flash_all.swf';
...
# template URL paths
my $domain = "http://jeffpflueger.com";
my $preview_template_path = $domain . "/content/dameasy_preview_template";
my $index_template_path = $domain . "/content/dameasy_index_template";
my $keyword_template_path = $domain . "/content/photo-mountains-keyword-search";
my $flash_template_path = $domain . "/content/dameasy_flash_template";
...
my $profile_path = '/usr/home/jeff/perl/sRGB.icm';
...
my $copyright_notice = "\x{a9}Jeff Pflueger";
...
##only do this if we are not running with images_only or pages_only
if ( !$images_only and !$pages_only ) {
	...
	# open the db connection to deal with Ubercart
	$global::dbh_2 = DBI -> connect( 
		"dbi:mysql:drupal_jeffpflueger",
	 	"jeff", "45grimrun") ||
	...
	while ( @row = $sth -> fetchrow_array ){
		...
		if  ( ( $sth2 -> rows ) == 0 ) {
			...
			# Ubercart: delete from the Ubercart Options:
			#print "\tDeleting Ubercart product option for filename $file_name\n";
			#$global::dbh_2 -> do ( "DELETE FROM uc_attribute_options, uc_product_options
						#		USING uc_attribute_options
						#		INNER JOIN uc_product_options
						#		WHERE uc_attribute_options.oid = uc_product_options.oid
						#		AND uc_attribute_options.name = '$file_name'" ) || die "Couldn't delete all from dameasy : $DBI::errstr";	
	...
	while ( @row = $sth -> fetchrow_array ) {
		...
		if  ( ( $sth2 -> rows ) == 0 ) {
			...
			# MAKE keywords LOWER CASE AND REMOVE REDUNDANT ENTRIES for keywords
			$keywords = lc( $keywords );
			my @keywords_arr = split ( /,\s*/,$keywords );
			my %keywords_hash   = map { $_, 1 } @keywords_arr;
       		@keywords_arr = keys %keywords_hash;
			@keywords_arr = sort(@keywords_arr);
			$keywords = '';
			...
			## add it to the ubercart options
			#print "\tAdding to Ubercart Options\n";
			#my $query2 = "INSERT INTO uc_attribute_options
			#(
			#aid,
			#oid,
			#name
			...
			#$global::dbh_2->do ( "$query2" ) ||die "$query2 failed: $DBI::errstr\n";
		
			#my $query2 = "INSERT INTO uc_product_options
			#(
			#nid,
			#oid
			#)
			...
			#$global::dbh_2->do ( "$query2" ) ||die "$query failed: $DBI::errstr\n";
	...
	$sth -> finish();
	$global::dbh_2 -> disconnect() || die "Failed to disconnect: $DBI::errstr\n";
} ## end section if not pages_only or images_only options
...
if ( $images_only ) {
	...
	while ( @row = $sth -> fetchrow_array ){
		...
		my $comp  = @row[3];
		my %exif_data = extract_exif ($ftp_file_path);
		my $rotate = $exif_data{"Orientation"};
		## extract the degrees to rotate the image
		$rotate =~ s/\(.*\)//g;  #remove everything in parenthesis like (1)
		$rotate =~ s/[^0-9]//g; #remove everything else but the number of degrees
		print "Recreating images for $ftp_file_path \n";
        #then do the ImageMagick
		imagemagick ( $ftp_file_path, $thumb, $preview, $comp, $rotate, $thumb_size, $preview_size, $comp_size );
		print "\n";
	}
...
if ( $change == 1 or $pages_only ) {
	if ( $pages_only ) {
		print "\n**I'm not writing anything in the database, just recreating the HTML pages**";
	}
	print "\n**Saving HTML Pages**\n";
	
	## now delete the old html pages
	print "\tDeleting all tmp HTML files\n";
	system ( "rm -r $tmp_html_path" ) || warn "can't delete all the directories $!";
	system ( "rm -r $tmp_html_keywords_path" ) || warn "can't delete all the directories $!";

	#create the directory that you just deleted
	system ( "mkdir -m 777 -p $tmp_html_path" );
	system ( "mkdir -m 777 -p $tmp_html_keywords_path" );
	print "\tSynching tmp html directory structure with the ftp directory\n";
	system ( "mtree -cdnp $ftp_path | ( cd $tmp_html_path ; mtree -U -e -q -n -d > /dev/null )" );
	...
	# now that you've deleted everything, copy over the narration, movie.swf and movie.flv for the flash movie
		...
		sub wanted_again {
			...
			if ( $File::Find::name =~ /narration\.mp3$/) {
				...
				$narration_path = $tmp_html_path . $narration_path;
			...
			if ( $File::Find::name =~ /movie\.swf$/) {
				...
				$movie_path = $tmp_html_path . $movie_path;
	...
	my $query = "SELECT preview, keywords, copyright, headline, file_name_root, id,  description, location, state, country, image_size, additional_path, file_name, instructions, ftp_file_path, gps_latitude_ref,
		...
		FROM dameasy ORDER BY additional_path, file_name ASC;";
	my $sth=$global::dbh->prepare ( "$query" ) ||die "$query - failed: $DBI::errstr\n";
	$sth -> execute();
	...
	for ( my $count = 0; $count < scalar ( @results ); $count++ ) {
		...
		$ftp_file_path =~ s/\/home\/dameasy\/ftp_images_new\///g;
	
		$preview = "$image_doc_path" . "$additional_path" . "$file_name_root" . "_" . "$id" . "_p.jpg";
		my $thumbnail ="$image_doc_path" . "$additional_path" . "$file_name_root" . "_" . "$id" . "_t.jpg";
		...
		my $additional_path_linked = "<a href='" . $html_doc_path . "index.html'>" . "Photo Collection/</a><br>";
		my $accumulation;
		my $spacer = '';
		...
		##create FACEBOOK like and comment
		my $facebook = "
		<div id='fb-root'></div>
<script>
  window.fbAsyncInit = function() {
    FB.init({appId: '152674834760196', status: true, cookie: true,
             xfbml: true});
  };
  (function() {
  ...
  ";
 		 # add like button
 		 # $facebook .="<fb:like></fb:like>";
 		 # add comments box
  		$facebook .="<fb:comments></fb:comments>";

	
		my $keyword_path = $keyword_doc_path . "index.html";
		my %tasks = (
			PREVIEW => "$preview",
			THUMBNAIL => "$thumbnail",
			KEYWORDS => "$keywords",
		...
		my $save_path = "$tmp_html_path" . "$additional_path" . "$file_name_root" . "_" . "$id" . ".html"; 
	...
	for ( @paths_unique ) {
		...
		my $query = "SELECT id, file_name_root, headline, description, keywords, additional_path FROM dameasy WHERE additional_path LIKE BINARY '$additional_path' ORDER BY file_name ASC";
		my $sth=$global::dbh->prepare ( "$query" ) ||die "$query - failed: $DBI::errstr\n";
		$sth -> execute();
		...
		#get the first image and description for the flash page
		my $first_keywords = $thumbnails[0][4];
		my $this_file_name_root = $thumbnails[0][1];
		my $this_id = $thumbnails[0][0];
		my $first_image ="$image_doc_path" . "$additional_path" . "$this_file_name_root" . "_" . "$this_id" . "_t.jpg";
		my $first_description = $thumbnails[0][3];
		...
		my $current_directory = "$tmp_html_path" . "$additional_path";
		my @directories;
		...
		my @additional_path_split = split(/\//, $additional_path);
		my $additional_path_linked = "<a href='" . $html_doc_path . "index.html'>" . "Photo Collection</a>";
		my $accumulation;
		my $spacer='';
		foreach (@additional_path_split) {
			$spacer = "&nbsp;&nbsp;>>&nbsp;&nbsp;";
		...
		#my $additional_path_unlinked;
		#foreach (@additional_path_split) {
		#	$spacer = "&nbsp;&nbsp;>>&nbsp;&nbsp;";
		#	$accumulation .= $_ . "/";
		#	$additional_path_unlinked .= $spacer . $_ ;
		#}

		## Create Flash all embed
		my $flash_all_url = "" . $html_doc_path . $additional_path . "flash_all.swf";
		my $flash_all = "<object classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000' codebase='http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,29,0' width='900' height='650'>
		  		<param name='movie' value='$flash_all_url'>
		  		<param name='quality' value='high'>
		  		<param name='LOOP' value='false'>
		  		<embed src='$flash_all_url' quality='high' pluginspage='http://www.macromedia.com/go/getflashplayer' type='application/x-shockwave-flash' width='900' height='650'></embed>
			</object>";
			
		## Create Flash embed
		my $flash_url = "" . $html_doc_path . $additional_path . "flash.swf";
		my $flash = "<object classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000' codebase='http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,29,0' width='900' height='650'>
		  		<param name='movie' value='$flash_url'>
		  		<param name='quality' value='high'>
		  		<param name='LOOP' value='false'>
		  		<embed src='$flash_url' quality='high' pluginspage='http://www.macromedia.com/go/getflashplayer' type='application/x-shockwave-flash' width='900' height='650'></embed>
			</object>";
	   ...
	   #save the flash all
	        my $flash_all_name = $current_directory;
			my $xml_all_name = $current_directory;
			copy( $swf_all_template_path, $flash_all_name ) or warn "Can't : copy( $swf_all_template_path, $flash_all_name ).";
			copy( $flash_xml_all_template_path, $xml_all_name ) or warn "Can't : copy( $flash_xml_all_template_path, $xml_all_name ).";
		...
		if ( $count > 0 ) {
			...
			## new Drupal Template way
			# new URL based template
			my $flash_template = get $flash_template_path;
  			die "Couldn't get $flash_template_path" unless defined $flash_template;
...
<object codebase='http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,29,0' width='900' height='650'>
	<param name='movie' value='" . $html_doc_path . $additional_path . "flash.swf'>
	<param name='quality' value='high'>
	<param name='base' value ='.'>
	<param name='LOOP' value='false'>
	<embed base='.' src='" . $html_doc_path . $additional_path . "flash.swf' quality='high' pluginspage='http://www.macromedia.com/go/getflashplayer' type='application/x-shockwave-flash' width='900' height='650'></embed>
</object>
</textarea>
";

		## add FACEBOOK to flash page
		my $flash_facebook = "
		<div id='fb-root'></div>
<script>
  window.fbAsyncInit = function() {
  ...
  ";
 		# add comments box
  		$flash_facebook .="<fb:comments></fb:comments>";
			...
			my %tasks = (
				...
				KEYWORDS_INDEX => "$keyword_path",
				FACEBOOK => "$flash_facebook",
				FIRST_IMAGE => "<img src='$first_image' alt='$first_description'>",
	...
	## Make Keyword Home page:
		...
		for ( @keywords_unique ) {
			...
			my $current_directory = "$tmp_html_keywords_path";
			...
			#save the page
			$keyword =~ s/\W/_/g;
			my $index_name = $tmp_html_keywords_path . $keyword . ".html";
			open INDEX_PAGE , ">$index_name" or die "Can't open file: $index_name to write to. $!";
			print INDEX_PAGE $index_template;
			close (INDEX_PAGE);
		}
		...
		#save the page
		my $index_name = $tmp_html_keywords_path . "index.html";
		open INDEX_PAGE , ">$index_name" or die "Can't open file: $index_name to write to. $!";
		print INDEX_PAGE $index_template;
		close (INDEX_PAGE);
	...
	## and move the newly created ones to the new place
	print "\nDeleting old HTML files and moving new ones in";
	system ( "mv $tmp_html_path $html_path" ) || warn "can't move $tmp_html_path to $html_path $!";
	system ( "mv $tmp_html_keywords_path $html_keywords_path" ) || warn "can't move $tmp_html_keywords_path to $html_keywords_path $!";
...
sub imagemagick {
	...
	if ( $width > $height ) {
		foreach my $imagesize ( @sizes ) {
			...
			my $ylocation = $newthumbheight/2-80;
