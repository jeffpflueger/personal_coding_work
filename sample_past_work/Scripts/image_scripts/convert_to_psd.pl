...
print "\n\nConvert to PSD\n
Takes every TIF file in a given directory,
saves it as a PSD file by the same name,
and erases the old TIF.
...
Path ( or 'quit' to exit)";
	...
	foreach my $filename (@files) {
		...
		if ( ($filename =~ /\.tif/) ) {
			...
			$x = $image->Set( magick=>"psd" );
				warn "magick setting $x" if "$x";
			## DELETE the old TIF ##
			print "\tDeleting $path"."\n";
			unlink "$path" or warn "Can't unlink the file: $path";
